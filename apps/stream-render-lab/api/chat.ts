...
export async function POST(request: Request): Promise<Response> {
  if (process.env.ENABLE_LIVE_API !== '1') {
    return json(403, { error: 'live_disabled' })
  }
  const key = process.env.DEEPSEEK_API_KEY
  if (key === undefined || key === '') return json(503, { error: 'live_not_configured' })
  ...
  const upstreamRequest = buildUpstreamRequest(parsed.value, key)
