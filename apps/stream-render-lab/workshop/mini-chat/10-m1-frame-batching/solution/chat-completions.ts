...
import type { ChatCompletionEvent } from '../../06-chat-completions/contract'
...
function appendChoice(output: ChatCompletionEvent[], choice: ChatCompletionChoice): void {
  if (choice.reasoning) output.push({ kind: 'reasoning-delta', text: choice.reasoning })
  if (choice.content) output.push({ kind: 'content-delta', text: choice.content })
  if (choice.finishReason) output.push({ kind: 'finish', reason: choice.finishReason })
}
...
function parseChoices(data: string): ChatCompletionChoice[] {
  ...
  return choices.map((choice) => {
    ...
    return {
      reasoning: optionalString(choice.delta.reasoning_content),
      content: optionalString(choice.delta.content),
