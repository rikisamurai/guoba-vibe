...
export async function renderSvg(data: string, opts: QrOptions = {}): Promise<string> {
  if (!data?.trim()) throw new Error('renderSvg: data is required')
...
export async function renderPng(data: string, opts: QrOptions = {}): Promise<Uint8Array> {
  if (!data?.trim()) throw new Error('renderPng: data is required')
  const { width, margin } = { ...DEFAULTS, ...opts }
