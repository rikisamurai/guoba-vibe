...
describe('Vault Store bootstrap and persistence', () => {
  ...
  it('loads a valid permissive v1 document without normalizing it', () => {
    const raw = serializedDocument({
      qrs: [{ id: 'q', url: 'raw', extra: true }],
      collections: [{ id: 'c', title: 'C' }],
      collectionItems: [
  ...
  it('loads an empty view for bad persisted data without overwriting it', () => {
    const storage = new MemoryVaultStorage('{bad json')
    const store = storeWith(storage)

    expect(store.getSnapshot().counts.qrs).toBe(0)
    expect(storage.raw).toBe('{bad json')
    expect(storage.writes).toHaveLength(0)
  })

  it('also preserves shape-invalid persisted data until the first mutation', () => {
    const invalid = JSON.stringify({ version: 1, qrs: [] })
    const storage = new MemoryVaultStorage(invalid)
    const store = storeWith(storage, { ids: ['created'] })

    expect(store.getSnapshot().counts.qrs).toBe(0)
    expect(storage.raw).toBe(invalid)
    store.qr.save({ url: 'xhsdiscover://rn/created' })
    expect(JSON.parse(storage.raw!).qrs).toHaveLength(1)
  })

  it('does not expose mutable nested values from permissive persisted data', () => {
    ...
    const store = storeWith(storage)
    const visible = store.getSnapshot().getQr('q')
    if (!visible) throw new Error('expected visible QR')
    const description: unknown = Reflect.get(visible, 'description')

    expect(Object.isFrozen(description)).toBe(true)
    if (description && typeof description === 'object') Reflect.set(description, 'flag', true)
    store.collection.save({ title: 'Unrelated' })
    expect(JSON.parse(store.transfer.exportJSON()).qrs[0].description).toEqual({ flag: false })
  })
...
describe('Vault views and transfers', () => {
  ...
  it('inspects invalid imports and applies reusable incoming-wins merge candidates', () => {
    ...
    expect(store.transfer.inspect('{')).toEqual({ kind: 'invalid' })
    expect(store.transfer.inspect(JSON.stringify({ version: 1, qrs: [] }))).toEqual({
      kind: 'invalid',
    })
